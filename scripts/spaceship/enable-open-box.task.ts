...
async function main() {
    ...
    console.log(`===== updateEnableOpenBox =====`);
    const { transactionHash } = await (await boxCt.connect(deployer).updateEnableOpenBox(true)).wait();
    ...
    await delayTime();
    try {
        const fileName = join(__dirname, `./outputs/${NODE_ENV}/enable-open-box.task.json`)
